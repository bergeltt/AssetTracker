...
void AssetTracker::turnOnGPS()
{
    ...
    useInternalGPSAntenna();
    delay(500);
}

void AssetTracker::turnOffGPS()
{
...
void AssetTracker::useInternalGPSAntenna()
{
	for(size_t i=0;i<sizeof(internalANT);i++)
	{
		Serial1.write(internalANT[i]); //send the command to gps modul
	}
}

void AssetTracker::useExternalGPSAntenna()
{
	for(size_t i=0;i<sizeof(externalANT);i++)
	{
...
int AssetTracker::readAccelX()
{
    accel.read();
    return accel.x;
}

int AssetTracker::readAccelY()
{
    accel.read();
    return accel.y;
}

int AssetTracker::readAccelZ()
{
    accel.read();
    return accel.z;
}

int AssetTracker::readAccelXYZmagnitude()
{
    accel.read();
